...
--[[---------------------------------------------------------
    Name: GetListenerTable
    Desc: Returns a table of listener hooks.
-----------------------------------------------------------]]
function GetListenerTable() return ListenerHooks end
...
--[[---------------------------------------------------------
    Name: AddListener
    Args: string hookName, any identifier, function func
    Desc: Add a hook to listen to the specified event gauranteed to run before hook.Add calls
-----------------------------------------------------------]]
function AddListener( event_name, name, func )
	if ( !isfunction( func ) ) then return end
	if ( !isstring( event_name ) ) then return end

	if (ListenerHooks[ event_name ] == nil) then
			ListenerHooks[ event_name ] = {}
	end

	ListenerHooks[ event_name ][ name ] = func
end
...
function RemoveListener( event_name, name )
	...
	ListenerHooks[ event_name ][ name ] = nil
...
function Call( name, gm, ... )
	...
	-- Run hooks
	--
	local ListenerTable = ListenerHooks[ name ]
	if ListenerTable != nil then
		for k, v in pairs( ListenerTable ) do
			v( ... )
		end
	end
