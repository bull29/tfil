-- https://facepunch.com/showthread.php?t=1522945&p=50526414&viewfull=1#post50526414
...
function PANEL:Init()
	self.Component = vgui.Create("DPanel", self)
	self.Component:SetPaintedManually(true)
	self.Component.Paint = nil
...
function PANEL:OnSizeChanged(w, h)
	self.Component:SetSize(self:GetWide(), self:GetTall())
...
function PANEL:Paint(w, h)
	...
	render.SetStencilReferenceValue(1)
	self.Component:SetPaintedManually(false)
	self.Component:PaintManual()
	self.Component:SetPaintedManually(true)
	if self.PaintCircle then
...
vgui.Register("DCirclePanel", PANEL)